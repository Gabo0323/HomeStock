...
export function DashboardScreen({ products, onNavigate }: DashboardScreenProps) {
            ...
            <TouchableOpacity style={styles.iconButton} onPress={() => onNavigate("Notifications")}>
              ...
              {alertProducts.length > 0 && (
                <View style={styles.badge}>
                  <Text style={styles.badgeText}>{alertProducts.length}</Text>
                </View>
              )}
            </TouchableOpacity>
          ...
          <View style={styles.alertsContainer}>
            ...
            {alertProducts.map((product) => (
              <View key={product.id} style={styles.alertItem}>
                <Image source={{ uri: product.image }} style={styles.alertImage} />
                <View style={styles.alertContent}>
                  <Text style={styles.alertProductName}>{product.name}</Text>
                  <Text style={styles.alertMessage}>
                    {product.status === "expired" && "Producto vencido"}
                    {product.status === "warning" && `Stock bajo: ${product.quantity} ${product.unit} restantes`}
                    {product.status === "low" && `Stock bajo: ${product.quantity} ${product.unit} restantes`}
                  </Text>
                </View>
                <View style={styles.alertBadge}>
                  <Text style={styles.alertBadgeText}>Stock {product.status === "expired" ? "vencido" : "bajo"}</Text>
                </View>
              </View>
            ))}
          </View>
